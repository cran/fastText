...
void Args::parseArgs(const std::vector<std::string>& args) {
  ...
  for (int ai = 2; ai < args.size(); ai += 2) {
    if (args[ai][0] != '-') {
      Rcpp::Rcout << args[ai - 1] << " " << args[ai] << std::endl;
      Rcpp::Rcout << "Provided argument without a dash! Usage:" << std::endl;
      printHelp();
      Rcpp::stop("EXIT_FAILURE -- args.cc file -- Args::parseArgs function");
    }
    try {
      if (args[ai] == "-h") {
        Rcpp::Rcout << "Here is the help! Usage:" << std::endl;
        printHelp();
        Rcpp::stop("EXIT_FAILURE -- args.cc file -- Args::parseArgs function");
        ...
        if (args.at(ai + 1) == "hs") {
          ...
          Rcpp::Rcout << "Unknown loss: " << args.at(ai + 1) << std::endl;
          printHelp();
          Rcpp::stop("EXIT_FAILURE -- args.cc file -- Args::parseArgs function");
        }
        ...
        Rcpp::Rcout << "Unknown argument: " << args[ai] << std::endl;
        printHelp();
        Rcpp::stop("EXIT_FAILURE -- args.cc file -- Args::parseArgs function");
      }
  ...
  if (input.empty() || output.empty()) {
    Rcpp::Rcout << "Empty input or output path." << std::endl;
    printHelp();
    Rcpp::stop("EXIT_FAILURE -- args.cc file -- Args::parseArgs function");
  }
